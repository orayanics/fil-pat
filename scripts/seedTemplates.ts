...
async function main() {
  ...
  // Check for existing templates for this clinician
  const existingStandardTemplate = await prisma.assessmentTemplate.findFirst({
    where: {
  ...
  const existingKidsTemplate = await prisma.assessmentTemplate.findFirst({
    where: {
  ...
  // Delete existing templates if found
  if (existingStandardTemplate) {
    console.log(`🗑️  Found existing standard template (ID: ${existingStandardTemplate.template_id}), deleting...`);
    await prisma.sessionItem.deleteMany({
      where: { template_id: existingStandardTemplate.template_id }
    });
    await prisma.assessmentTemplate.delete({
      where: { template_id: existingStandardTemplate.template_id }
    });
    console.log('   ✅ Deleted existing standard template');
  }

  if (existingKidsTemplate) {
    console.log(`🗑️  Found existing kids template (ID: ${existingKidsTemplate.template_id}), deleting...`);
    await prisma.sessionItem.deleteMany({
      where: { template_id: existingKidsTemplate.template_id }
    });
    await prisma.assessmentTemplate.delete({
      where: { template_id: existingKidsTemplate.template_id }
    });
    console.log('   ✅ Deleted existing kids template');
    console.log('');
  }
